...
class RocketCalendar {
    ...
    options;
    input;
    disabledDates = [new Date(2023, 2, 5)];
    ...
    init = async () => {
        if (this.options.locale && this.options.locale !== 'en') {
            const localeImported = await import(`./locales/${this.options.locale}`);
            if (localeImported)
                this.locale = { ...localeImported };
        }
        if (this.input.value)
    ...
    printValue = () => {
        if (this.selectedDate) {
            if (this.options.type === 'time') {
                this.input.value = this.selectedDate.toLocaleTimeString(this.locale.name, {
                    hour: '2-digit',
                    minute: '2-digit'
                });
            }
                ...
                this.input.value = this.selectedDate.toLocaleDateString(this.locale.name, {
                    year: 'numeric',
                    month: '2-digit',
                    day: '2-digit'
                });
    ...
    generateDays = (month, year) => {
        ...
        for (let i = 1; i <= 42; i++) {
            ...
            const isDisabled = this.disabledDates.some((date) => this.isSameDay(date, dayDate));
            const isToday = this.isSameDay(this.today, dayDate);
            const isSelected = this.selectedDate && this.isSameDay(this.selectedDate, dayDate);
            const day = document.createElement('div');
            day.classList.add('calendar__day');
            if (i >= firstDay && monthDay <= numberOfDays) {
                day.innerText = monthDay.toString();
                if (isToday)
                    day.classList.add('calendar__day--is-today');
                if (isSelected)
                    day.classList.add('calendar__day--is-selected');
                if (isDisabled)
                    day.classList.add('calendar__day--is-disabled');
    ...
    subscribeInputEvents = () => {
        ...
        document.addEventListener('click', (event) => {
            if (this.input.contains(event.target) ||
                this.calendarComponent?.contains(event.target)) {
                return;
            }
            this.isOpened = false;
        });
